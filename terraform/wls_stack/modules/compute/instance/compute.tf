...
resource "oci_core_instance" "wls_no_jrf_instance" {
  ...
  shape_config {
    #Optional
    ocpus = length(regexall(var.instance_shape, "^.*Flex")) > 0 ? var.wls_ocpu_count: lookup(data.oci_core_shapes.oci_shapes[count.index % length(local.ad_names)].shapes[0], "ocpus")
  }
