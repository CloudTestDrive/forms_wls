...
module "network-vcn-config" {
  ...
  appdb_vcn_peering            = local.appdb_vcn_peering
}
...
module "network-bastion-subnet" {
  ...
  subnetCount         = ! local.assign_weblogic_public_ip && var.bastion_subnet_id == "" && var.is_bastion_instance_required && var.existing_bastion_instance_id == "" ? 1 : 0
...
module "compute" {
  ...
  wls_cluster_name              = "cluster_forms"
...
module "policies" {
  ...
  appdb_network_compartment_id   = local.appdb_network_compartment_id
  appdb_existing_vcn_add_seclist = var.appdb_existing_vcn_add_seclist
  appdb_is_atp_db                = var.app_atp_db_id != ""
  wls_admin_password_ocid        = var.wls_admin_password_ocid
  app_db_password_ocid           = var.app_db_password_ocid
  atp_db_password_ocid           = var.atp_db_password_ocid
  oci_db_password_ocid           = var.oci_db_password_ocid
  idcs_client_secret_ocid        = var.idcs_client_secret_ocid
  instance_ocids                 = compact(concat(module.bastion-compute.id, module.compute.InstanceOcids))
}
